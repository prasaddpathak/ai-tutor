...
EduLibreX is built with a clean, layered architecture optimized for edge deployment:

**Frontend (React + TypeScript)**
- Modern web interface with Material-UI components
- Camera integration for face recognition login
- Progressive Web App for offline functionality

**Backend (FastAPI)**
- RESTful API server handling all educational services
- Authentication, curriculum management, and chat services
- Lightweight Python server optimized for Jetson hardware

**AI Engine (Ollama + Gemma 3n)**
- Local AI inference using Ollama runtime
- Gemma 3n model for content generation and tutoring
- GPU-accelerated processing on Jetson's Ampere architecture

**Data Storage (SQLite)**
- Local database storing all student data and AI-generated content
- Face recognition encodings and learning progress
- Chapter content, quizzes, and educational materials generated by Gemma
- Zero external dependencies - everything stays on device
...
sequenceDiagram
    ...
    B->>AI: Generate Chapter Content
    AI->>B: Return Generated Chapter
    B->>DB: Store Chapter Data
